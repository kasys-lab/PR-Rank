...
def run(config: OmegaConf):
    ...
    global_model_dir_path = Path(config.global_model.global_model_dir_path)

    if not global_model_dir_path.exists():
        ...
        logger.info("Global model already exists. Skipping...")
